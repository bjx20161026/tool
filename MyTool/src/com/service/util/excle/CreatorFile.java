package com.service.util.excle;
...
public class CreatorFile {
	...
	public byte[] Creator(String[] heads,String[] head, List<Map<String,String>> list, String type) throws IOException {
		if (type.equals("csv"))
			return CreatCsv(heads,head,list);
		else
			return createExcelFile(heads,head,list, type);
	}

	public byte[] CreatCsv(String[] heads, String[] head,List<Map<String,String>> list) throws IOException {
	...
	public byte[] createExcelFile(String[] heads, String[] head,List<Map<String,String>> list, String type) throws IOException {
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
...
//			String[] head = {"工单流水号","主题","派单人","派单部门","派单时间","处理时限","热点名称","屏蔽开始时间","屏蔽结束时间","属地分公司","行政区域","附件ftp信息","工单处理状态","是否成功","工单类型","nasid","完成时间","回单信息","工单包含的处理项","操作时间","本地附件路径","失败原因","操作人"};
			...
			for (int rowNum = 1; rowNum < list.size()+1; rowNum++) {
				Map<String, String> map = list.get(rowNum - 1);
